...
from input10 import raw

clean = [int(adapter) for adapter in raw.split('\n')]
adapters = sorted(clean)
...
index = 0
outlet = max(adapters) + 3
adapters = [0] + adapters + [outlet]

while index < (len(adapters) - 1):
    diffs[str(adapters[index + 1] - adapters[index])] += 1
    index += 1
...
valid = set({})
invalid = set({})
demo = [0] + [1, 4, 5, 6, 7, 10, 11, 12, 15, 16, 19] + [22]
demo2 = [0, 1, 2, 3, 4, 7, 8, 9, 10, 11, 14, 17, 18, 19, 20, 23, 24, 25, 28, 31, 32, 33, 34, 35, 38, 39, 42, 45, 46, 47, 48, 49, 52]
checks = { 'found': 0 }

def recurse(collection):
    for x in range(1, len(collection) - 1):
        if collection[x+1] - collection[x-1] > 3:
            # print('skip')
            continue
        working_list = collection.copy()
        working_list.remove(collection[x])
        string_version = '-'.join(map(str,working_list))
        if string_version in valid:
            # print('found')
            ...
            # print('new')
            valid.add(string_version)
            recurse(working_list)

recurse(demo)
# print(valid)
print(len(valid) + 1, checks['found'])
