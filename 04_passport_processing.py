...
from input04 import raw

clean = [passport.replace('\n',' ').split(' ') for passport in raw.split('\n\n')]

def deconstruct_passport(passport):
    deconstructed = {}
...
passports = [deconstruct_passport(passport) for passport in clean]
...
def validate_passport(passport):
    ...
    if int(passport['eyr']) < 2020 or int(passport['eyr']) > 2030:
        return False
    if (passport['hgt'][-2:] != 'cm' and passport['hgt'][-2:] != 'in') or not isinstance(int(passport['hgt'][0:len(passport['hgt']) - 2]), int):
        return False
    if passport['hgt'][-2:] == 'cm' and (int(passport['hgt'][0:len(passport['hgt']) - 2]) < 150 or int(passport['hgt'][0:len(passport['hgt']) - 2]) > 193):
        return False
    if passport['hgt'][-2:] == 'in' and (int(passport['hgt'][0:len(passport['hgt']) - 2]) < 59 or int(passport['hgt'][0:len(passport['hgt']) - 2]) > 76):
        return False
    if passport['hcl'][0] != '#' or len(passport['hcl'][1:]) != 6:
        return False
    if passport['ecl'] not in ecl_values:
        return False
    if len(passport['pid']) != 9 or not isinstance(int(passport['pid']), int):
        return False
    return True
