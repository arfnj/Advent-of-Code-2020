...
from input01 import raw

clean = [int(x) for x in raw.split('\n')]
clean = sorted(clean)
...
while low < high:
    ...
    if sum < 2020:
        low += 1
    if sum == 2020:
        print(f"Part 1 answer: {left * right}")
        break
    if sum > 2020:
        high -= 1
...
"""
--- Part Two ---
The Elves in accounting are thankful for your help; one of them even offers you a starfish coin they had left over from a past vacation. They offer you a second
...
for value in range(len(clean)):
    iterator = 1
    deficit = 2021
    if str(clean[value]) in have_it:
        print(f"Part 2 answer: {clean[value] * have_it[str(clean[value])]}")
        break 
    while deficit > 0:
        combo = clean[value] + clean[value + iterator]
        deficit = 2020 - combo
        have_it[str(deficit)] = clean[value] * clean[value + iterator]
        iterator += 1
