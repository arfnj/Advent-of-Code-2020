...
from input07 import raw

clean = raw.split('\n')
rules = {}

def parser(rule):
    parsed = rule.split(' contain ')
    container = parsed[0].split(' bags')[0]
    details = parsed[1].split(', ')
    details_obj = {}
    total_within = 0
    for detail in details:
        split_detail = detail.split(' ')
        color = f"{split_detail[1]} {split_detail[2]}"
        if split_detail[0] == 'no':
            continue
        details_obj[color] = int(split_detail[0])
        total_within += int(split_detail[0])
        if color in rules:
            rules[color]['found_in'].add(container)
        else:
            rules[color] = { 'found_in': set({container}) }
    if container in rules:
        rules[container]['contains'] = details_obj
        rules[container]['total_within'] = total_within
    else:
        rules[container] = {
            'contains': details_obj,
            'found_in': set({}),
...
# print(rules['vibrant bronze'])

part1 = 0

bags1 = set({})
collection1 = ['shiny gold']

while len(collection1):
    new_collection = []
    for color in collection1:
        # print(color)
        for bag in rules[color]['found_in']:
            if bag not in bags1:
                part1 += 1
                bags1.add(bag)
                new_collection.append(bag)
    collection1 = new_collection.copy()
...
collection2 = [ 'shiny gold' ]

while len(collection2):
    new_collection = []
    for color in collection2:
        part2 += rules[color]['total_within']
        for bag in rules[color]['contains']:
            for x in range(rules[color]['contains'][bag]):
                new_collection.append(bag)
    collection2 = new_collection.copy()
