...
to the instruction immediately below it, and jmp -20 would cause the instruction 20 lines above to be executed
next.
...
import re
from input08 import raw

instructions = [ (lambda line: (line[0:3], line[4], int(line[5:])))(line) for line in raw.split('\n')]

visited1 = set({})
accumulator1 = 0
index1 = 0

while index1 not in visited1:
    visited1.add(index1)
    if instructions[index1][0] == 'jmp':
        index1 += instructions[index1][2] if instructions[index1][1] == '+' else (instructions[index1][2] * -1)
        continue
    if instructions[index1][0] == 'acc':
        accumulator1 += instructions[index1][2] if instructions[index1][1] == '+' else (instructions[index1][2] * -1)
    index1 += 1
...
while index2 not in visited2 and index2 < len(instructions):
    visited2.add(index2)
    command = instructions[index2][0]
    if not changed and index2 not in attempts and command != 'acc':
        command = 'jmp' if command == 'nop' else 'nop'
        attempts.add(index2)
        changed = True
    if command == 'jmp':
        index2 += instructions[index2][2] if instructions[index2][1] == '+' else (instructions[index2][2] * -1)
    if command == 'acc':
        accumulator2 += instructions[index2][2] if instructions[index2][1] == '+' else (instructions[index2][2] * -1)
    index2 += 1 if command != 'jmp' else 0
    if index2 in visited2:
        index2 = 0
        visited2.clear()
        accumulator2 = 0
        changed = False
