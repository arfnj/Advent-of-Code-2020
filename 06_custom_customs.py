...
from input06 import raw

sets = [len(set((lambda group : group.replace('\n',''))(group))) for group in raw.split('\n\n')]
...
def formatter(group):
    obj = {
        'count': group.count('\n') + 1,
        'raw': (lambda group : group.replace('\n',''))(group)
    }
    obj['uniques'] = set(obj['raw'])
    return obj

part2 = [formatter(group) for group in raw.split('\n\n')]

everyone = 0

for group in part2:
    for question in group['uniques']:
        if group['raw'].count(question) == group['count']:
            everyone += 1
